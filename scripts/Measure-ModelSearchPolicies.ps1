...
$offsets = if ($Samples -eq 'Development') { @(30000000, 150000000, 30000000) } else { @(70000000, 250000000, 70000000) }
$types = @('temple', 'shipwreck', 'village')
...
try {
    for ($i=0; $i -lt $types.Count; $i++) {
        ...
        $null = & $search -Type $type -Sisters 65536 -FamilyCap 4 -Target 1000000 -Families 1000000000 `
            -Seconds $SecondsPerType -Stream $offsets[$i] -OutputFile $bank -Java $Java
        ...
        $summary += [pscustomobject]@{
            type = $type; samples = $Samples; streamOffset = $offsets[$i]
