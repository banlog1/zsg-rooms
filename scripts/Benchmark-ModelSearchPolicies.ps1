...
param(
    ...
    [ValidateSet('All', 'temple', 'shipwreck', 'village')][string]$Type = 'All',
    [string]$Java = 'java'
)
...
try {
    foreach ($case in $cases) {
        $stream = 400000000L + $SampleIndex * 1000000000L
        ...
        $null = & $search -Type $case.type -Sisters $case.sisters -FamilyCap $case.cap -Target 1000000 `
            -Families 1000000000 -Seconds $SecondsPerCase -Stream $stream -OutputFile $bank -Java $Java
        ...
        $record = [pscustomobject]@{
            type=$case.type; sisters=$case.sisters; familyCap=$case.cap; streamOffset=$stream
