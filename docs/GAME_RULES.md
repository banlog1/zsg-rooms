...
The new ZSG Rooms Temple/Village **bank profiles** also have built-in
[surface-pool preparation](SEED_BANK_TERRAIN.md), independently of these toggles.
...
relocates it. X/Z are projected into the Nether and centered on a block; the
original spawn height is retained. FSG currently returns no intended
portal-building coordinates, so this does not anchor to a filter's lava pool
or ravine yet.

Minecraft still searches for an existing portal or creates one using its normal
terrain checks and fallback platform. The initial creation orientation uses an
isolated seed-derived roll; the original RNG roll is also consumed so subsequent
portal RNG progression is preserved. Runners building entry portals at different
positions/heights therefore use the same destination search and creation inputs.
Matching placement assumes matching destination terrain and existing portals;
this does not overwrite player-modified terrain to enforce an identical result.

Consumption is saved per player in the world, so reopening that save does not
reapply the adjustment. A fresh same-seed world reset repeats the first-entry
placement. Non-portal teleports do not consume it. Return trips and all later
entries use vanilla linking; no forced return pairing is installed. Consequently,
a first portal built far from the common reference might not link back to the
runner's original Overworld portal.

When preloading is enabled, it warms the common reference chunk for the first
entry, then switches back to the player's projected coordinates for later entries.
No Nether chunks are generated by this rule during startup: only the reference
and saved consumption state are loaded then. Normal portal placement work occurs
on entry.
